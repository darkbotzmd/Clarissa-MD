...
let handler = async (m, { conn, participants, usedPrefix, command, args, isOwner, isPrems, isAdmin }) => {
	...
	if (isOwner || isAdmin || isPrems) {
		...
		if (global.db.data.users[who].banned == true) throw `Dia sudah di *mute* sebelumnya.`
		try {
...
handler.menugroup = ['diem @tag <timer>','sutt @tag <timer>']
handler.tagsgroup = ['owner']
handler.command = /^(diem|silent|sutt)$/i
