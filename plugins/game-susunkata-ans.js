...
export async function before(m) {
    ...
    if (!(id in this.susunkata))
        return conn.sendButton(m.chat, 'Soal itu telah berakhir', botdate, null, buttonsusunkata, m)
    if (m.quoted.id == this.susunkata[id][0].id) {
        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text)
        if (isSurrender) {
            clearTimeout(this.susunkata[id][3])
            delete this.susunkata[id]
            return conn.sendButton(m.chat, '*Yah Menyerah :( !*', botdate, null, buttonsusunkata, m)
        }
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            conn.sendButton(m.chat, `*Benar!*\n+${this.susunkata[id][2]} XP`, botdate, null, buttonsusunkata, m)
            clearTimeout(this.susunkata[id][3])
            delete this.susunkata[id]
        } else if (similarity(m.text.toLowerCase(), json.jawaban.toLowerCase().trim()) >= threshold)
            m.reply(`*Dikit Lagi!*`)
        else
            conn.sendButton(m.chat, `*Salah!*`, botdate, null, [
                ['Hint', '/hsus'],
                ['Nyerah', 'menyerah']
            ], m)
    }
